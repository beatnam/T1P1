...
public class ManagerService {
	...
	public List<Map<Object, Object>> listOR(PageDTO pageDTO) {
		...
		return managerMapper.listOR(pageDTO);
	}
	...
	public int countOR() {
		// TODO Auto-generated method stub
		return managerMapper.countOR();
	}

	public List<Map<Object, Object>> listREBoard(PageDTO pageDTO1) {
		// 10개씩 잘랐을 떄 1페이지 내용 디비에서 가져오기 위해서
		// 시작하는 행 번호 구하기
		// 페이지번호(pageNum) 페이지크기(pageSize) => 시작하는 행번호(startRow)
		// 1 10 => 0 * 10 + 1 => 0 + 1 => 1
		// 2 10 => 1 * 10 + 1 => 10 + 1 => 11
		// 3 10 => 2 * 10 + 1 => 20 + 1 => 21
		int startRow = (pageDTO1.getCurrentPage() - 1) * pageDTO1.getPageSize() + 1;
		...
		// 11 10 => 11 + 10 -1 => 20
		// 21 10 => 21 + 10 -1 => 30
		int endRow = startRow + pageDTO1.getPageSize() - 1;

		// limit #{startRow - 1}, #{pageSize}
		pageDTO1.setStartRow(startRow - 1);
		pageDTO1.setEndRow(endRow);
		return managerMapper.listREBoard(pageDTO1);
	}
	...
	public List<Map<Object, Object>> listRIBoard(PageDTO pageDTO2) {
		...
		// limit #{startRow - 1}, #{pageSize}
		pageDTO2.setStartRow(startRow - 1);
		pageDTO2.setEndRow(endRow);
		return managerMapper.listRIBoard(pageDTO2);
	}

	public void deleteRE(int num) {
		
		managerMapper.deleteRE(num);
		
	}

	public void deleteRI(int num) {
		managerMapper.deleteRI(num);
